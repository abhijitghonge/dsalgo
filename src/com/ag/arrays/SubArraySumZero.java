...
public class SubArraySumZero {
    public static void main(String[] args) {
        ...
        //System.out.println(sumzero.solve(A));

        System.out.println(sumzero.solve1(A));
    }
    ...
    public int solve1(int[] A){
        ...
        System.out.printf("length: %d, pfsums: %d, distinctSums: %d, count: %d %n", A.length,pfsums.length, distinctSums.size(), count);

        return (A.length - distinctSums.size()) + (count) ;
    ...
    private long[] createPrefixSums(int[] A){
        ...
        for(int i = 1; i < A.length; i++){
            pfsums[i] = (pfsums[i-1] % 1000000007 + A[i]) % 1000000007;
        }
